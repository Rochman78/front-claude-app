...
- **Refacto 02/07/2026** : auparavant on demandait les 4 ANGLES aux sommets. Trop peu fiable au rapporteur sur le terrain (une petite erreur d'angle explose la surface), et sur-déterminé (Bretschneider n'en réclame que 2 opposés). UNE diagonale = geste plus simple (mesure de longueur au mètre ruban), même pouvoir géométrique, plus la forme réelle. Tous les blocs × 10 agents mis à jour, ancien texte "4 angles" supprimé.
...
- Calcul de surface après réception du croquis : la diagonale d divise le quadrilatère en 2 triangles (côtés a-b-d et c-d_bas-d). Appliquer Héron directement sur chacun : `S = √(s(s−x)(s−y)(s−z))` avec `s = (x+y+z)/2`. Surface totale = S1 + S2. Arrondir UNIQUEMENT le résultat final au dixième de m².
