...
- 4 côtés seuls ne suffisent JAMAIS à calculer la surface d'un quadrilatère quelconque (forme non unique). **Donnée obligatoire à demander : les 4 ANGLES aux sommets** (en degrés, même approximatifs au rapporteur ou estimés à l'œil). La diagonale n'est PAS une alternative acceptée — toujours les 4 angles.
- **INTERDICTION ABSOLUE** : ne jamais conclure « ces mesures ne forment pas un trapèze géométriquement réalisable » / « incohérence géométrique » sur la seule base des 4 côtés. C'est faux dans 99 % des cas — la forme existe en quadrilatère quelconque, il manque juste les angles.
- Calcul de surface après réception du croquis : à partir d'un angle θ entre les côtés a et b, reconstituer la diagonale par loi des cosinus (`diag = √(a² + b² − 2·a·b·cos θ)`), puis décomposer en 2 triangles et appliquer Héron sur chacun.
- Cas Dominique Delpit 18/06/2026 (`cnv_1lnflc5z`, LFC) : client demande filet « type trapèze » 4,4×3×5,32×1 m. Sous hypothèse trapèze (bases parallèles), géométriquement impossible — VRAI mais TROMPEUR. En quadrilatère quelconque, forme fabricable, il manque juste les angles. Claude a écrit au client « ces mesures ne forment pas un trapèze géométriquement réalisable » → cliente perd confiance et abandonne. Encodé × 10 agents (bloc dédié entre LECTURE DES CROQUIS et TVA) + grille `prix-ht-sur-mesure.txt` × 9 boutiques.
